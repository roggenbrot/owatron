...
function createMainWindow() {
  if (!mainWindow) {
    ...
    mainWindow.webContents.setWindowOpenHandler((details) => {
      if (details.url.startsWith('https://teams.microsoft.com')) {
        console.log('Open ' + details.url + ' in microsoft teams');
        shell.openExternal(details.url.replace('https', 'msteams'));
        return {
          action: 'deny',
        };
      } else if (details.url.startsWith(config.get('url', 'https://outlook.office.com/mail'))) {
        return {
          action: 'allow',
      ...
      console.log('Open ' + details.url + ' in external application');
...
ipcMain.handle('resetCookies', async () => {
  await mainWindow?.webContents.session.clearStorageData({
    storages: ['appcache', 'filesystem', 'serviceworker', 'cachestorage', 'cookies'],
  });
  mainWindow?.close();
  mainWindow = undefined;
...
ipcMain.handle('resetStorage', async () => {
  await mainWindow?.webContents.session.clearStorageData({
    storages: ['appcache', 'filesystem', 'serviceworker', 'cachestorage'],
  });
});
